...
export class SiparislerComponent implements OnInit {
  ...
  Duzenle(kayit: Siparis) {
    ...
    this.dialogRef.afterClosed().subscribe((d) => {
      if (d) {
        ...
        kayit.projeSiparisId = d.projeSiparisId;
  ...
  Sil(kayit: Siparis) {
    ...
    this.confirmdialogRef.componentInstance.dialogMesaj = kayit.siparisKodu + " Kodlu " + kayit.projeSiparisId + '  Projenin Siparişi Silinecektir Onaylıyor Musunuz?';

    this.confirmdialogRef.afterClosed().subscribe(d=>{
      if (d) {
        this.apiServis.SiparisSil(kayit.projeSiparisId).subscribe((s:Sonuc)=>{
          this.alert.AlertUygula(s);
          if (s.islem) {
            this.SiparisListe();
          }
